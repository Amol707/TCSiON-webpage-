...
//remove pre-existing content
  ...
  for(var i=0;i<colleges.length;i++){
    var value = colleges[i];  
    //if college name and caste match and the annual fee is less than 60% of annaulIncome
    if(value.College_Name == collegeName && value.Caste == casteName && 0.6*parseInt(value.Annual_Fee)<=annualIncome){
      if(flag){
